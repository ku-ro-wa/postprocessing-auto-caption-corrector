...
from caption_checker.evaluation import ScoredCase, is_cold_flag, load_corpus, score
from caption_checker.models import Flag


def _flag(detector: str, candidates: list[str] | None = None, span: str = "cubernetes") -> Flag:
    return Flag(
        span=span,
        global_indices=[0],
        cue_index=1,
...
def test_score_counts_true_positive_when_span_and_candidate_match() -> None:
    ...
    flags = {"a.srt": [_flag("split_word", ["Kafka"], span="cough ka")]}
    report = score(cases, flags)
...
def test_score_counts_false_negative_when_candidate_wrong() -> None:
    cases = [ScoredCase("a.srt", "cough ka", "should-flag", "Kafka")]
    flags = {"a.srt": [_flag("split_word", ["Kubernetes"], span="cough ka")]}
    report = score(cases, flags)
    assert report.false_negatives == 1
...
def test_score_counts_false_positive_when_known_good_span_flagged() -> None:
    cases = [ScoredCase("a.srt", "raft", "should-not-flag")]
    flags = {"a.srt": [_flag("oov", span="raft")]}
    report = score(cases, flags)
...
def test_score_reports_cold_flag_rate_over_all_produced_flags() -> None:
    ...
    flags = {
        "a.srt": [
            _flag("split_word", ["Kafka"], span="cough ka"),
            _flag("oov", span="mystery"),
        ]
    }
    report = score(cases, flags)
